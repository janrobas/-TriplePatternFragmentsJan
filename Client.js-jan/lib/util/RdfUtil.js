...
/** Removes the given bindings to the triple or graph pattern, returning an unbound copy thereof. */
util.removeBindings = function (bindings, pattern) {
  if (typeof pattern.map === 'function')
    return pattern.map(function (p) { return util.removeBindings(bindings, p); });

  return _.mapValues(pattern, function (value, key) {
    for (var b in bindings) {
      if (value === bindings[b]) return b;
    }
    return value;
  });
};
...
namespace('fri', 'http://www.fri.uni-lj.si/#', [
  'predicateFilter', 'data'
]);
