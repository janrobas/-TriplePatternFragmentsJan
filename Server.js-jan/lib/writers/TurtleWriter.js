...
TurtleWriter.prototype.writeFragment = function (destination, tripleStream, settings) {
  ...
  var writer = this._createWriter(destination, settings),
      metadataWritten = false, triplesWritten = false, filterWritten = false, filterData = "",
  ...
  tripleStream.on('metadata', function (metadata) {
    ...
    if(triplesWritten && filterWritten) {
      if(filterData) {
        writer.meta(fragment.url, fri + 'predicateFilter', '_:filter');
        writer.meta('_:filter', fri + 'data', '"' + filterData + '"^^' + xsd + 'base64Binary');
        //writer.meta('_:filter', fri + 'predicates', '"' + "filter.predicates" + '"^^' + xsd + 'base64Binary');
      }
      writer.end();
    }
  });

  tripleStream.on('filter', function (filter) {
    // Write AMQ filter
    if (filter) {
    ...
    if(triplesWritten && metadataWritten) {
      if(filterData) {
        writer.meta(fragment.url, fri + 'predicateFilter', '_:filter');
        writer.meta('_:filter', fri + 'data', '"' + filterData + '"^^' + xsd + 'base64Binary');
        //writer.meta('_:filter', fri + 'predicates', '"' + "filter.predicates" + '"^^' + xsd + 'base64Binary');
  ...
  tripleStream.on('end',  function ()  {
     ...
     if(filterWritten && metadataWritten) {
       if(filterData) {
         writer.meta(fragment.url, fri + 'predicateFilter', '_:filter');
         writer.meta('_:filter', fri + 'data', '"' + filterData + '"^^' + xsd + 'base64Binary');
         //writer.meta('_:filter', fri + 'predicates', '"' + "filter.predicates" + '"^^' + xsd + 'base64Binary');
