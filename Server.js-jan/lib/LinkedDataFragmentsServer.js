...
function LinkedDataFragmentsServer(options) {
  ...
  server._filter = _.assign({ type: require('./filters/PredicateFilter.js') });
...
LinkedDataFragmentsServer.prototype._handleRequest = function (request, response) {
  ...
  // Zahtevek na ta URL vrne dosedanjih število zahtevkov in to številko ponastavi
  if(request.url == "/probe") {
    response.writeHead(405, { 'Content-Type': MIME_PLAINTEXT });
    response.end("" + (this._zahtevki-1));
    this._zahtevki = 0;
    return;
  }
/*
  if(request.url == "/traffic") {
    response.writeHead(405, { 'Content-Type': MIME_PLAINTEXT });
    response.end("" + (this._zahtevki-1));
    this._zahtevki = 0;
    return;
...
LinkedDataFragmentsServer.prototype._sendFragment = function (request, response, writerSettings) {
  ...
  var self = this, queryResult = datasourceSettings.datasource.select(query, onError),
      metadata = this._createFragmentMetadata(request, query, datasourceSettings, queryResult);
...
LinkedDataFragmentsServer.prototype._createFragmentMetadata =
  function (request, query, datasourceSettings, queryResult) {
  ...
  query.patternString = '{ ' +
    ...
    // TODO: shranjevati mogoče nekam drugam (ne kar v file)
    // če imamo vse troje (subject, predicate in object), potem gre za poizvedbo po točno določenem trojčku (membership query)
  /*  if(query.subject && query.predicate && query.object) {
        // take poizvedbe lahko filtriramo, zato beležimo, za kater predikat gre
        fs.appendFile('predikati.txt', query.subject + ", " + query.predicate + ", " + query.object + "\n", (err) => {
          if (err) throw err;
        });
    }*/
    ...
    if (this._filter) {
      ...
      if (!query.predicate)
        variable = 'predicate';
      else if (!query.object && !query.subject)
        variable = 'object subject';
...
if (!query.predicate)
  ...
  variable = 'subject';*/
      ...
      // konstrukt tajp
      queryResult.on('metadata', function (metadata) {
        var totalCount = metadata.totalCount;

        self._filter.type(datasourceSettings.datasource, query, variable, totalCount, function (error, filterData) {
          if (error) {
            console.error(error);
            queryResult.emit('filter', null);
          } else if(!filterData) {
            queryResult.emit('filter', null);
          } else {
            queryResult.emit('filter', filterData);
          }
        });
      });
    }
